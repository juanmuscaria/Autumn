...
package com.juanmuscaria.autumn.messages.standard;

import com.juanmuscaria.autumn.messages.AbstractResourceBasedMessageSource;
import com.juanmuscaria.autumn.messages.MessageSource;
import com.juanmuscaria.autumn.messages.util.DefaultPropertiesPersister;
import com.juanmuscaria.autumn.messages.util.PropertiesPersister;
...
/**
 * {@link MessageSource} implementation
...
public class ReloadableResourceBundleMessageSource extends AbstractResourceBasedMessageSource {
    ...
    private List<String> fileExtensions = Arrays.asList(".properties", XML_EXTENSION);
     ...
     */
    public void setFileEncodings(@Nullable Properties fileEncodings) {
     ...
     * <p>The default is a DefaultResourceLoader.
     * @see DefaultResourceLoader
     */
    ...
    protected List<PropertiesHolder> collectPropertiesToMerge(Locale locale) {
        String[] basenames = getBasenameSet().toArray(new String[0]);
        ...
        if (resource != null) {
                ...
                try {
                    ...
                    if (propHolder != null && propHolder.getFileTimestamp() == fileTimestamp) {
                        if (logger.isLoggable(Level.FINE)) {
                            logger.fine("Re-caching properties for filename [" + filename + "] - file hasn't been modified");
                        }
                    ...
                    if (logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, resource + " could not be resolved in the file system - assuming that it hasn't changed", ex);
                    }
                    fileTimestamp = -1;
                }
            }
            try {
                ...
                if (logger.isLoggable(Level.WARNING)) {
                    logger.log(Level.WARNING, "Could not parse properties file [" + resource.getFilename() + "]", ex);
                }
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("No properties file found for [" + filename + "]");
            }
    ...
    protected Properties loadProperties(Resource resource, String filename) throws IOException {
        ...
        try (InputStream is = resource.getInputStream()) {
            ...
            if (resourceFilename != null && resourceFilename.endsWith(XML_EXTENSION)) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Loading properties [" + resource.getFilename() + "]");
                }
                ...
                if (encoding != null) {
                    if (logger.isLoggable(Level.FINE)) {
                        logger.fine("Loading properties [" + resource.getFilename() + "] with encoding '" + encoding + "'");
                    }
    ...
    public void clearCache() {
        logger.fine("Clearing entire resource bundle cache");
